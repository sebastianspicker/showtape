...
test('imports, matches, manually changes, and exports a playlist @screenshots', async ({
  ...
  await page.getByRole('textbox', { name: 'Search Apple Music' }).fill("Can't Buy Me Love");
...
test('reflows without horizontal overflow at phone, 200%, 400%, and desktop widths', async ({
  page,
}, testInfo) => {
...
test('supports keyboard completion, focus transitions, cancellation, and preserved edits', async ({
  ...
  await page.keyboard.press('Enter');
  await expect(page.getByRole('heading', { name: 'Match songs' })).toBeFocused();
  await expect(page.getByText('3 of 3 songs matched')).toBeVisible();

  const changeButton = page.getByRole('button', { name: "Change match for Can't Buy Me Love" });
  await changeButton.focus();
  await page.keyboard.press('Enter');
  const searchInput = page.getByRole('textbox', { name: 'Search Apple Music' });
...
test('keeps long setlist metadata and songs operable at 320px', async ({ page }, testInfo) => {
